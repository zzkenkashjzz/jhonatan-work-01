...
const mkpsWithSameListPriceAttb = [sellerMarketplaces.AMAZON, sellerMarketplaces.AMAZON_CA]
...
export default ({ getOrder, defaultCode, listingId, externalId, orderRetrieved, path, tab, propertiesInfo, canEditItem, setIsPack, form, currentTab, justSent, isMain }) => {
    ...
    const setDataForListingMarketplace = (tab, mkt, orderRetrieved) => {
        ...
        if (product) {
            !product?.attributes?.list_price ? mkpsWithSameListPriceAttb.includes(mkt) ? (product.attributes.list_price = { value: product?.price, currency: marketplaceCurrency[mkt] }) :
        ...
        if (variants?.length > 0) {
            for (const [idx, variant] of variants?.entries()) {
                ...
                !variants[idx]?.attributes?.list_price ? mkpsWithSameListPriceAttb.includes(mkt) ? (variants[idx].attributes.list_price = { value: variant?.price, currency: marketplaceCurrency[mkt] }) :
