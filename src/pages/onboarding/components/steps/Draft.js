...
import { imageSectionEnum } from '../../../../utils/const.js';
...
const Draft = ({ supportRequests, setSupportRequests, listingId, prevStep, updateStep, selected, setSelected }) => {
    ...
    const exportExcel = async () => {
        setLoadingApproveListing(true);
            try {  
                const { data } = await partnerApi.exportListing(session.userInfo.partner_id[0], listingId);
                const filename = `Amazon - ${draftRetrieved.listingPerMarketplace.Amazon.product.defaultCode}.xlsx`;
